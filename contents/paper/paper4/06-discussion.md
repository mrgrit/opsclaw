...
## 6.5 Bastion 소스 분석을 통한 하네스 융합

Claude Code의 소스코드(TypeScript ~1,884파일)를 분석한 결과, 서버 사이드 하네스(OpsClaw)에 적용 가능한 6가지 패턴을 도출하였다:

1. **동적 프롬프트 조합** (prompt_engine): Claude Code의 시스템 프롬프트가 7개 섹션으로 동적 조합되는 패턴을 OpsClaw에 적용
2. **라이프사이클 훅** (hook_engine): PreToolUse/PostToolUse 패턴을 pre_dispatch/post_dispatch로 변환
3. **도구 검증** (tool_validator): Zod 스키마 기반 검증을 JSON Schema로 구현
4. **비용 추적** (cost_tracker): 세션별 토큰 추적을 프로젝트별 비용 추적으로 확장
5. **다층 퍼미션** (permission_engine): 4-모드 권한 시스템을 RBAC+Policy+Approval+Risk로 확장
6. **메모리 관리** (memory_manager): 4-유형 메모리를 자동 추출+LRU로 고도화

이는 클라이언트 하네스의 설계 패턴을 서버 하네스에 **역수입(back-port)**한 사례로, 두 패러다임이 상호 학습할 수 있음을 실증한다.
